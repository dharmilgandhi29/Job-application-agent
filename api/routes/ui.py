...
function detailHTML(j){
  ...
  const jd = esc(stripHtml(j.description)).slice(0, 1200);
  const seen = j.first_seen ? new Date(j.first_seen).toLocaleDateString(undefined,{month:'short',day:'numeric',year:'numeric'}) : 'unknown';
  const pills = [];
  if(j.role_type) pills.push(`<span class="pill">${esc(j.role_type)}</span>`);
  if(j.seniority_fit) pills.push(`<span class="pill">seniority: ${esc(j.seniority_fit)}</span>`);
  if(j.sponsor_new != null) pills.push(`<span class="pill">${j.sponsor_new} new H-1B</span>`);
  return `
    <div class="dsec">
      <h4>Mr. Jober's read</h4>
      <p>${esc(j.reasoning) || 'No notes on this one.'}</p>
    </div>
    <div class="dsec">
    ...
    <div class="dsec">
      <h4>The posting <span style="font-weight:600;text-transform:none;letter-spacing:0;color:#B8AE9A">· first spotted ${seen}</span></h4>
      <div class="jd">${jd}${jd.length >= 1200 ? '\\u2026' : ''}</div>
